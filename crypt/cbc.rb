...
module CBC
  ...
  def decrypt_stream(cryptStream, plainStream, with_padding = true)
    # Cypher-block-chain mode
  ...
  def decrypt_string(cryptText, with_padding = true)
    cryptStream = StringIO.new(cryptText)
    plainStream = StringIO.new('')
    decrypt_stream(cryptStream, plainStream, with_padding)
